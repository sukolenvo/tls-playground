...
const std::array<std::array<unsigned char, 16>, 16> sbox = {
		std::array<unsigned char, 16>{ 0x63, 0x7c, 0x77, 0x7b, 0xf2, 0x6b, 0x6f, 0xc5,
...
std::array<std::array<unsigned char, 16>, rounds + 1> build_schedule_key(const std::array<unsigned char, key_length> &key)
{
	std::array<std::array<unsigned char, 4>, (rounds + 1) * 4> buffer{};
	for (size_t i = 0; i < key_length; i += 4)
	{
		buffer.push_back({ key[i], key[i + 1], key[i + 2], key[i + 3] });
	}
	...
	std::array<std::array<unsigned char, 16>, rounds + 1> result{};
	for (size_t i = 0; i < result.size(); ++i) {
		std::copy(buffer.at(i * 4).cbegin(), buffer.at(i * 4).cend(), result[i].begin());
		std::copy(buffer.at(i * 4 + 1).cbegin(), buffer.at(i * 4 + 1).cend(), result[i].begin() + 4);
		std::copy(buffer.at(i * 4 + 2).cbegin(), buffer.at(i * 4 + 2).cend(), result[i].begin() + 8);
		std::copy(buffer.at(i * 4 + 3).cbegin(), buffer.at(i * 4 + 3).cend(), result[i].begin() + 12);
	}
	return result;
}