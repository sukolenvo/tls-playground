...
TEST_CASE("DSA")
{
	static const std::vector<unsigned char> g{
	...
	static const std::vector<unsigned char> q{
	...
	auto signature = dsa.sign_sha256(message, private_key);
	const auto &rData = signature.r.data();
	const auto &sData = signature.s.data();
	CAPTURE(hexStr(rData.begin(), rData.end()), hexStr(sData.begin(), sData.end()));
	BigNumber public_key({
	...
	REQUIRE(dsa.verify_sha256(message, signature, public_key));
}